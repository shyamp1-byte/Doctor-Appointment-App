...
const getListofDctors = async (req, res) => {
  console.log("Get all getListofDctors");
  let filteredData = [];
  try {
    let alldoctors = await docModel.find({});
    // HERE I NEED TO ADD FILTER TO DISPLAY THE ID AND NAME


    alldoctors.map((doctor) => {
      console.log(doctor);
      filteredData.push({ DID: doctor.DID, name: doctor.fullName.firstName, specialization: doctor.specialization,yearsOfExpirience:doctor.yearsOfExpirience,educationalQualification:doctor.educationalQualification });
    })
